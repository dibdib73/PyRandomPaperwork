...
def iter_through_full_list(filesInFolder: List[str]):
...
def first_split(firstBreak: List[str]) -> list[str]:
...
def add_to_db(contentAdd) -> None:
    if len(contentAdd) == 3:
        ...
        exeStatment = (
            "INSERT INTO Scanned_paperwork (Type, Paperwork_name, Date_scanned, Date_of_paperwork)"
            "VALUES (?, ?, ?, ?);"
        )
        ...
        print(
            "Name must start with originization name, then type of paperwork,\nand end with date. Must be separated with an underscore"
        )
